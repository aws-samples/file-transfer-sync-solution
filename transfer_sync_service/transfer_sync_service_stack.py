...
with open(
	'./configuration/solution_parameters/parameters.json', encoding='utf8'
) as solution_parameters:
...
class TransferSyncServiceStack(Stack):
	...
	if not os.path.exists(
		f'transfer_sync_service/lambda/boto3_{boto_version}_lambda_layer'
	):
	...
	def create_scheduler(
		...
		"""
		Creates a scheduler for the Transfer Sync service.

		Args:
		    config: Configuration dictionary containing Name, Schedule, and SyncSettings
		    connector_id: ID of the Transfer Family connector
		    state_machine_arn: ARN of the Step Functions state machine
		    scheduler_role_arn: ARN of the IAM role for the scheduler
		    report_bucket_name: Name of the S3 bucket for reports

		Returns:
		    CfnSchedule: The created scheduler object
		"""
		cron_expression = self.cron_helper(cron_str)
		scheduler_config = {
			'Connector': connector_id,
			'ReportBucket': report_bucket_name,
			'Name': config_name,
			'SyncSettings': sync_setting,
			'Schedule': cron_expression,
		}

		safe_remote_folder = sync_setting['RemoteFolders']['Folder'][1:].replace(
			'/', '-'
		)

		scheduler_name = f'{config_name}-{safe_remote_folder}'
		return scheduler.CfnSchedule(
			self,
			f'transfer-sync-schedule-{scheduler_name}',
			...
			target=scheduler.CfnSchedule.TargetProperty(
				...
				input=json.dumps(scheduler_config),
			),
		)
	...
	def create_ssm_parameter(
		...
		parameter_name = f'{config_name}-{safe_remote_folder}'
		return ssm.StringParameter(
			self,
			f'transfer-sync-ssm-{parameter_name}',
			string_value=json.dumps(parameter_config),
			parameter_name=f'/transfer-sync/{parameter_name}',
	...
	def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
		...
		monitoring = MonitoringFacade(
			self,
			'TransferSyncSolutionMonitoring',
		...
		report_kms_key = kms.Key(self, 'ReportKmsKey', enable_key_rotation=True)

		# Create an S3 bucket for the SFTP Transfer Family Connector reports
		report_bucket = s3.Bucket(
			self,
			'TransferSyncReportBucket',
		...
		remote_server_list_lambda = lambda_.Function(
			self,
			'TransferSyncRemoteServerListLambda',
			runtime=lambda_.Runtime.PYTHON_3_12,
			code=lambda_.AssetCode.from_asset(
				os.path.join(
					os.getcwd(), 'transfer_sync_service/lambda/remote_server_list'
				)
			),
		...
		get_list_status_lambda = lambda_.Function(
			self,
			'TransferSyncGetListStatusLambda',
			runtime=lambda_.Runtime.PYTHON_3_12,
			code=lambda_.AssetCode.from_asset(
				os.path.join(
					os.getcwd(), 'transfer_sync_service/lambda/get_list_status'
				)
			),
		...
		sync_files = lambda_.Function(
			self,
			'TransferSyncFilesLambda',
			runtime=lambda_.Runtime.PYTHON_3_12,
			code=lambda_.AssetCode.from_asset(
				os.path.join(os.getcwd(), 'transfer_sync_service/lambda/sync_files')
			),
		...
		scheduler_role = iam.Role(
			self,
			'TransferSyncSchedulerRole',
			assumed_by=iam.ServicePrincipal('scheduler.amazonaws.com'),
		)

		# Create Step Functions tasks
		task_prepare_input = sfn.Pass(
			self,
			'TaskPrepareInput',
			parameters={
				...
				'Schedule.$': '$.Schedule',
				'FromTimestamp.$': "$.['FromTimestamp', 'false']",
			},
		)

		task_remote_server_list_lambda = sfn_tasks.LambdaInvoke(
			self,
			'TaskRemoteServerListLambda',
		...
		wait_list = sfn.Wait(
			self, 'WaitList', time=sfn.WaitTime.duration(Duration.seconds(5))
		)
		...
		choice_definition.when(condition_list_pending, wait_list).when(
			condition_recursive_list, task_remote_server_list_lambda
		).otherwise(task_sync_files_lambda)

		chain_definition = (
			task_prepare_input.next(task_remote_server_list_lambda)
			.next(wait_list)
			.next(task_get_list_status_lambda)
			.next(choice_definition)
		)
		...
		for file in self.files('./configuration/sftp/'):
			with open(
				f'./configuration/sftp/{file}', encoding='utf8'
			) as service_config:
				...
				secret = secretmanager.Secret(
					self,
					f'transfer-sync-secret-{config["Name"]}',
				...
				transfer_access_role = iam.Role(
					self,
					f'transfer-sync-access-role-{config["Name"]}',
					description=config['Description'],
				...
				transfer_logging_role = iam.Role(
					self,
					f'transfer-sync-logging-role-{config["Name"]}',
					description=config['Description'],
					assumed_by=iam.ServicePrincipal('transfer.amazonaws.com'),
				...
				connector = transfer.CfnConnector(
					self,
					f'transfer-sync-connector-{config["Name"]}',
					...
					tags=[CfnTag(key='Name', value=f'sync-connector-{config["Name"]}')],
				)
				...
				for sync_setting in config['SyncSettings']:
					...
					transfer_access_role.add_to_policy(
						iam.PolicyStatement(
							...
							resources=[
								f'arn:aws:s3:::{sync_setting["LocalRepository"]["BucketName"]}/*'
							],
						)
					)
					transfer_access_role.add_to_policy(
					...
					if 'KmsKeyArn' in sync_setting['LocalRepository']:
						transfer_access_role.add_to_policy(
							iam.PolicyStatement(
								...
								resources=[
									f'{sync_setting["LocalRepository"]["KmsKeyArn"]}'
								],
							)
						)
					...
					sync_files.role.add_to_policy(
						iam.PolicyStatement(
							...
							resources=[
								f'arn:aws:s3:::{sync_setting["LocalRepository"]["BucketName"]}/{sync_setting["LocalRepository"]["Prefix"]}/*'
							],
						)
					)
					# Get schedule from sync_setting if exists, otherwise from config
					schedule = sync_setting.get('Schedule', config.get('Schedule'))
					# if 'Schedule' in sync_setting:
					if str.lower(schedule) in ['ondemand', 'manual', '', 'on-demand']:
						# Create Parameter
						self.create_ssm_parameter(
							config_name=config['Name'],
							sync_setting=sync_setting,
							setting_id=str(setting_id),
							connector_id=connector.attr_connector_id,
					...
					# else:
					#     if str.lower(config['Schedule']) in ['ondemand', 'manual', '', 'on-demand']:
					#         # Create Parameter
					#         self.create_ssm_parameter(
					#             config_name=config['Name'],
					#             sync_setting=sync_setting,
				...
				transfer_logging_role.add_to_policy(
					iam.PolicyStatement(
						...
						resources=[
							f'arn:aws:logs:{self.region}:{self.account}:log-group:/aws/transfer/*'
						],
					)
				)
		...
		monitoring.add_large_header('Lambda Functions').add_small_header(
			'Remote Server List'
			...
			pattern='ERROR',
			alarm_friendly_name='error logs',
		).add_small_header('Get List Status').monitor_lambda_function(
			lambda_function=get_list_status_lambda
		).monitor_log(
			...
			pattern='ERROR',
			alarm_friendly_name='error logs',
		).add_small_header('Sync Remote Files').monitor_lambda_function(
			lambda_function=sync_files
		).monitor_log(
			...
			pattern='ERROR',
			alarm_friendly_name='error logs',
		).add_large_header('Step Functions').monitor_step_function(
			state_machine=state_machine,
			add_failed_execution_count_alarm={
				'Error': ErrorCountThreshold(max_error_count=1, datapoints_to_alarm=1)
			},
		).add_large_header('S3 Buckets').monitor_s3_bucket(bucket=report_bucket)

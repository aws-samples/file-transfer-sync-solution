...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
	...
	try:
		...
		safe_remote_folder = event['SyncSettings']['RemoteFolders']['Folder'][
			1:
		].replace('/', '-')
		...
		safe_time_compare = calculate_safe_time_compare(
			schedule, start_time, event['FromTimestamp']
		)

		# Paginate through S3 objects
		s3_pages = paginator.paginate(
			Bucket=event['ReportBucket'], Prefix=output_directory_path
		)
...
def check_first_copy(event: Dict[str, Any], safe_remote_folder: str) -> bool:
	...
	try:
		...
		s3.head_object(
			Bucket=event['SyncSettings']['LocalRepository']['BucketName'],
			Key=f"{event['SyncSettings']['LocalRepository']['Prefix']}/{safe_remote_folder}.flag",
		)
		...
		logger.info(
			'This is the first copy for this destination, everything will be copied.'
		)
		return True


def process_s3_pages(
	s3_pages: Any, event: Dict[str, Any], first_copy: bool, safe_time_compare: Any
) -> None:
	"""
	...
	for s3_page in s3_pages:
		for s3_object in s3_page.get('Contents', []):
			try:
				...
				logger.error(
					f"Error processing S3 object: {s3_object['Key']}", exc_info=True
				)
...
def should_transfer_file(
	...
	if safe_time_compare < file_time:
		try:
			...
			obj = s3.head_object(
				Bucket=event['SyncSettings']['LocalRepository']['BucketName'],
				Key=f"{event['SyncSettings']['LocalRepository']['Prefix']}{file['filePath']}",
			)
			if obj['LastModified'] < file_time:
				logger.info(
					f"File {file['filePath']} has been modified since last copy."
				)
				return True
			else:
				logger.info(
					f"File {file['filePath']} has not been modified since last copy."
				)
				return False
		except botocore.exceptions.ClientError:
...
def calculate_safe_time_compare(
	schedule: str, start_time: datetime, from_timestamp: dict
) -> datetime:
	"""
	...
	Args:
	    ...
	    from_timestamp (dict): The 'FromTimestamp' value from the event data.

	Returns:
	    datetime: The safe time to compare against.
	"""
	if schedule == 'on-demand':
		if 'FromTimestamp' in from_timestamp:
			return datetime.fromtimestamp(
				int(from_timestamp['FromTimestamp']), tz=timezone.utc
			)

...
def lambda_handler(event, context):
	...
	if event['SyncSettings']['RemoteFolders']['Recursive']:
	...
	logger.info(
		...
		extra={
			...
			'loop_counter': event['LoopCounter'],
		},
	)
	return event
	...
	for output_object in output_objects:
		try:
			...
			if list_result['paths']:
				event['RemoteFolders'].extend(
					[path['path'] for path in list_result['paths']]
				)

			# Remove the processed object from OutputObjects
			event['OutputObjects'].remove(output_object)
			logger.info(
				'Processed output object', extra={'output_object': output_object}
			)
		except ClientError as e:
			logger.warning(
				'Failed to process output object',
				extra={'output_object': output_object, 'error': str(e)},
			)
...
def process_single_listing(event, output_objects):
	...
	try:
		...
		if error_code == '404':
			...
			logger.warning(
				'Object not available yet', extra={'object_key': output_objects[0]}
			)
			event = update_loop_counter(event)
		elif error_code == '403':
			# Access denied error
			logger.error(
				'Access denied',
				extra={'object_key': output_objects[0], 'error': str(e)},
			)
		else:
...
def update_loop_counter(event):
	...
	if event['LoopCounter'] > MAX_LOOP_COUNT:
		...
		event['WaitingList'] = False
		logger.error(
			f'Loop counter exceeded maximum allowed attempts, please check the Connector logs, connector-id: {event['Connector']}'
		)

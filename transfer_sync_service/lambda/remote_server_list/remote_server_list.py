...
def lambda_handler(event, context):
	...
	try:
		...
		if 'RemoteFolders' in event:
			...
			remote_folder = event['SyncSettings']['RemoteFolders']['Folder']
			process_directory_listing(event, remote_folder, output_directory_path)
		...
		logger.info(
			...
			extra={'output_objects': event['OutputObjects']},
		)
		return event
...
def process_directory_listing(event, remote_folder, output_directory_path):
	...
	try:
		...
		logger.info(
			f'Directory listing started for {processed_remote_folder}',
			extra={
				'remote_folder': processed_remote_folder,
		...
		logger.error(
			f'AWS Transfer client error for {processed_remote_folder}: {str(e)}',
			extra={
				'remote_folder': processed_remote_folder,
